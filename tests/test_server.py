...
# ── Vault auto_sync tests ─────────────────────────────────────────────────────

def test_vault_auto_sync_default_true(client):
...
def test_vault_update_auto_sync_false(client):
    """Should be able to set auto_sync to False."""
    kb = client.post("/api/knowledge-bases", json={"name": "kb1", "description": "", "type": "vault"}).json()
    vault_created = client.post(f"/api/knowledge-bases/{kb['id']}/vault",
                                json={"type": "local", "local_path": "/tmp/vault"}).json()
    vault_id = vault_created["id"]

    # Update auto_sync to False
    r = client.put(f"/api/vaults/{vault_id}/auto-sync", json={"auto_sync": False})
    assert r.status_code == 200
    vault = r.json()
    assert vault["auto_sync"] is False
...
def test_vault_update_auto_sync_true(client):
    """Should be able to set auto_sync back to True."""
    kb = client.post("/api/knowledge-bases", json={"name": "kb1", "description": "", "type": "vault"}).json()
    vault_created = client.post(f"/api/knowledge-bases/{kb['id']}/vault",
                                json={"type": "local", "local_path": "/tmp/vault"}).json()
    vault_id = vault_created["id"]
    ...
    # Set to False first
    client.put(f"/api/vaults/{vault_id}/auto-sync", json={"auto_sync": False})

    # Then set back to True
    r = client.put(f"/api/vaults/{vault_id}/auto-sync", json={"auto_sync": True})
    assert r.status_code == 200
    vault = r.json()
    assert vault["auto_sync"] is True
