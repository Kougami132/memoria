...
def parse_message(event: dict) -> QQInboundMessage | None:
    ...
    if event_type == "C2C_MESSAGE_CREATE":
        ...
        if not event_id or not user_openid or not content:
            return None
        return QQInboundMessage(event_id, "c2c", user_openid, user_openid, content, message_id=event_id)
    if event_type == "GROUP_AT_MESSAGE_CREATE":
        ...
        if not event_id or not group or not user or not content:
            return None
        return QQInboundMessage(event_id, "group", group, user, content, group_openid=group, group_member_openid=user, message_id=event_id)
    return None
